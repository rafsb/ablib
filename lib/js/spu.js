...
Object.prototype.anime = function(o=null, len=SP_ANIMDURATION, fn = null, trans = 'ease') {
    ...
    len/=1000;
    this.style.transition = "all " + len.toFixed(1) + "s "+trans;
    //console.log("all " + len.toFixed(1) + "s "+trans);
    for(let i in o){
        switch(i){
...
HTMLElement.prototype.appear = function(len = SP_ANIMDURATION){
    this.anime({display:'inline-block',opacity:1},len);
}

HTMLElement.prototype.desappear = function(len = SP_ANIMDURATION, remove = false){
    this.anime({opacity:0},len);
    setTimeout(function(el){ remove ? el.parent().removeChild(el) : el.style.display = "none"; },len++,this,remove);
}
...
class SPUME {
    ...
    notify(n, c = ["white", "black"]){
        ...
        toast.onclick = function() { this.desappear(400,true); };
        ...
        for (let i=notfys.length; i--;) notfys[i].anime({ top: ( ( toast.offsetHeight + 8 ) * i + 16) + "px", opacity: 1 }, 220);
        toast.dataset.delay = setTimeout(function() { toast.desappear(400,true); }, 4000);
    }

    modal(obj=null, fn=null, template="default", scrolls=false){
        this.load('../etc/modal.php',{data:obj,template:template,scrolls:scrolls},null,fn);
    }
