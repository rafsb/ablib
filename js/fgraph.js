...
class FGraph {
    ...
    draw(o){
        if(this.target&&this.node){
            ...
            , series   = o.series.extract(serie => serie.extract(s => (s.content || { _: app.iterate(0,10,x=>0.0001) }).array())[0])
            , labels   = o.series.extract(serie => serie.extract(s => (s.content || { NULL: null }).keys())[0])[0]
            ...
            entities.each((name, idx) => {
                ...
                // Calculate last position and trend_positions
                serie.pop();
                var k = (o.trend_periods || 0) + 1;
                do{
                    serie.push(serie.calc(TREND))
                } while (k--);

                ymax = Math.max(ymax, serie.last()*1.1)
    ...
    constructor(o){
        ...
        this.colors = o.colors || PRISM.array();
